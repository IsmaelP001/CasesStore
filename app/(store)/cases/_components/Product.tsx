...
const Product = ({
  ...
  const { id, name, collection, price, isFavorite, material, coverImage } =
    product;
  ...
  return (
    <article
      className={cn("snap-start relative rounded-md  flex-grow", className)}
    >
      <button
        disabled={isPending}
      ...
      >
        ...
        <div className="px-5 grid place-content-center text-center mt-1">
          <p className="font-semibold text-sm capitalize tracking-wider text-nowrap truncate">
            {name}
          </p>
          <p className="text-gray-500 text-xs capitalize">{collection?.name}</p>
          <p className="font-semibold text-xs capitalize">{material?.name}</p>
          <p className="font-medium text-xs">{formatPrice(price!)}</p>
        </div>
        {showCartBtn && (
