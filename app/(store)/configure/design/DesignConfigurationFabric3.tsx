...
const DesignConfigurator = () => {
  ...
  //   },
  //   onSuccess: () => {
  //     router.push(`/case/configure/preview?id=${"test"}`);
  //   },
  // });

  console.log("stickers selected", stickerOptions.selectedStickerId);
  console.log("sticker ", stickerOptions.stickersInScreen);
  async function saveConfiguration() {
    try {
      ...
      if (uploadedImage) {
        fabric.Image.fromURL(uploadedImage?.URL, (oImg) => {
          oImg.set({
            left: actualX,
            top: actualY,
          });
          oImg.scaleToWidth(renderedDimension.width);
          oImg.scaleToHeight(renderedDimension.height);
      ...
      //sticker
      for (const sticker of stickerOptions.stickersInScreen) {
        console.log("sticker screen", sticker);
        const leftSticker = sticker.x - leftOffset;
        const topSticker = sticker.y - topOffset;
        fabric.Image.fromURL(sticker.image.src, (oImg) => {
          oImg.set({
            left: leftSticker,
            top: topSticker,
          });
          oImg.scaleToWidth(sticker.width);
          oImg.scaleToHeight(sticker.height);
      ...
      const text = new fabric.Text(customText, {
        left: fontX,
        top: fontY,
        fontFamily: AlbaSlabOne.style.fontFamily,
      ...
      // const blob = editor.canvas.getElement().toBlob(async (blob) => {
  ...
  const deleteSticker = (id) => {
    setStickersOptions((prev) => {
      const updatedItems = prev.stickersInScreen.filter(
    ...
    <>
      ...
      <div className="relative  mt-10 grid grid-cols-1 lg:grid-cols-3 mb-20 pb-10 px-5">
        ...
        >
          ...
          {customText.length > 0 && (
            ...
            >
              <div
                className={` ${AlbaSlabOne.className} relative m-0 p-0  w-full h-full`}
              >
                <h2
                  className="absolute flex justify-center w-full h-full items-center m-0  box-border text-nowrap  object-contain overflow-hidden  text-white font-black  tracking-widest"
                  style={{
                ...
                >
                  <span className=" text-inherit h-auto m-0 text-center" ref={customTextContainerRef}> {customText}</span>
                </h2>
              </div>
            </Rnd>
          )}
      ...
      <div className="relative w-60 bg-opacity-50 pointer-events-none aspect-[896/1831]">
        ...
        >
          <FabricJSCanvas
            onReady={onReady}
            className={`w-full h-full border-2 border-primary z-[999]`}
          />
        </AspectRatio>
      </div>
    </>
